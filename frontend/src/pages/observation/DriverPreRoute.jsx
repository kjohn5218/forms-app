...
const OBSERVATION_ITEMS = [
  ...
  { name: 'mapsUnfamiliarLocations', label: 'Driver mapped unfamiliar locations that looked potentially hazardous' },
  { name: 'asksDispatchQuestions', label: 'Driver asks dispatch any questions needed to clarify delivery information' },
  { name: 'inspectsLoad', label: 'Driver inspects the load to ensure freight is strapped as needed. Verifies hazmat is strapped and labels are visible' },
  { name: 'loadsMechanicalAssist', label: 'Driver chooses, loads, and secures necessary mechanical assist equipment ie: pallet jacks, hand carts' },
  { name: 'closesDoorsChain', label: 'Driver closes trailer door (if a roll door), closes dock door, and hangs safety chain' }
]
...
const DriverPreRoute = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const submissionId = generateFormId('DPR')

    const formData = {
      ...data,
      formSubtype: 'driver-pre-route',
      result,
      submissionId
    }
    ...
    try {
      ...
      if (response.ok) {
        navigate('/success', {
          state: { submissionId, formType: 'Delivery Driver Pre-route Observation' }
        })
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <FormContainer onSubmit={handleSubmit(onSubmit)}>
        ...
        <FormSection title="Pre-route Observation Items">
