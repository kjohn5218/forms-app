...
const OBSERVATION_ITEMS = [
  { name: 'utilizesProperPPE', label: 'Utilizes Proper PPE' },
  { name: 'usesProperLifting', label: 'Uses Proper Lifting Techniques' },
  { name: 'communicatesHazards', label: 'Communicates Hazards Immediately' },
  { name: 'awareOfSurroundings', label: 'Aware of Surroundings' },
  { name: 'usesHorn', label: 'Uses horn to notify other forklifts/pedestrians of their presence' },
  { name: 'cautionLoadingUnloading', label: 'Uses caution while loading and unloading trailer' },
  { name: 'maintainsForkliftHeight', label: 'Maintains 4-6 inch forklift height while in motion' }
]
...
const DockSafety = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const submissionId = generateFormId('DSO')

    const formData = {
      ...data,
      formSubtype: 'dock-safety',
      result,
      submissionId
    }
    ...
    try {
      ...
      if (response.ok) {
        navigate('/success', {
          state: { submissionId, formType: 'Dock Safety Observation' }
        })
      } else {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <FormContainer onSubmit={handleSubmit(onSubmit)}>
        <FormSection title="Basic Information">
          ...
          />
          <TextInput
            label="Location/Dock Area"
            name="location"
            register={register}
            errors={errors}
            required
            placeholder="Specific dock area"
          />
        </FormSection>
