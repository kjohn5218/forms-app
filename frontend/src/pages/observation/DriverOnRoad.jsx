...
const OBSERVATION_ITEMS = [
  { name: 'goalsBeforeBack', label: 'Driver GOALs prior to every back' },
  { name: 'backsOnlyWhenNecessary', label: 'Backs only when necessary' },
  { name: 'backsImmediatelyMinimizes', label: 'Backs immediately & minimizes the back' },
  { name: 'cautiousAroundBillboards', label: 'Driver is cautious around "billboards"' },
  { name: 'maintainsAdequateSpace', label: 'Driver maintains adequate space around their equipment' },
  { name: 'stopsAtSignals', label: 'Driver stops completely at all traffic signals or signs' },
  { name: 'signalsInAdvance', label: 'Driver signals well in advance' }
]
...
const DriverOnRoad = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const submissionId = generateFormId('DOR')

    const formData = {
      ...data,
      formSubtype: 'driver-on-road',
      result,
      submissionId
    }
    ...
    try {
      ...
      if (response.ok) {
        navigate('/success', {
          state: { submissionId, formType: 'Driver On-Road Observation' }
        })
      } else {
