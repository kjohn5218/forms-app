...
const OBSERVATION_ITEMS = [
  { name: 'looksBeforeStepping', label: 'Looks Before Stepping' },
  { name: 'establishesFirmFooting', label: 'Establishes Firm Footing' },
  { name: 'adjustsToConditions', label: 'Adjusts to changing conditions' },
  { name: 'walksBriskPace', label: "Don't run, walk at a brisk pace" },
  { name: 'staysOffFreight', label: 'Stays off of freight and elevated surfaces' },
  { name: 'maintainsCleanArea', label: 'Maintains clean work area' },
  { name: 'threePointsContact', label: 'Utilizes 3 Points of Contact' }
]
...
const SlipsTripsAndFalls = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const submissionId = generateFormId('STF')

    const formData = {
      ...data,
      formSubtype: 'slips-trips-falls',
      result,
      submissionId
    }
    ...
    try {
      ...
      if (response.ok) {
        navigate('/success', {
          state: { submissionId, formType: 'Slips, Trips and Falls Observation' }
        })
      } else {
...
export default SlipsTripsAndFalls
